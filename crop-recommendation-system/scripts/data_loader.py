...
class CropDataLoader:
    ...
    def load_data_from_file(self, use_test_data=False):
        ...
        url = self.test_csv if use_test_data else self.train_csv
...
@st.cache_data
def load_and_preprocess_data(train_csv="train.csv",
                           test_csv="test.csv"):

    """Load and preprocess data with caching for Streamlit"""
    loader = CropDataLoader(train_csv, test_csv)
    
    try:
        train_df, test_df, combined_df = loader.load_both_datasets()
        X_train, X_test, y_train, y_test, X_scaled, y_encoded = loader.preprocess_data(train_df, test_df)
        return loader, combined_df, X_train, X_test, y_train, y_test, X_scaled, y_encoded
    except Exception as e:
        print(f"Failed to load from files s: {e}")
        print("Trying fallback approach...")
        # Fallback to single dataset approach
        df = loader.load_data_from_file(use_test_data=False)
